...
import main
from main import LexemeExchanger
...
def descobrir_dados():
    ...
    retornado = main.getInterfaces(main.tokensGerente)
    dataframes_lista = [pd.DataFrame(x) for x in retornado]
    print(f"[GUI] {len(dataframes_lista)}")

    #global dataframe
    #dataframe = pd.DataFrame(main.getDevices(main.tokensGerente, dataframes_lista[0][0]))


    #withIp = next(i for i in  if len(i[1]) > 1)
...
(main.um, main.outro) = socketpair()
main.tokensGerente = LexemeExchanger(main.um)
threading.Thread(target=main.agenciando).start()
